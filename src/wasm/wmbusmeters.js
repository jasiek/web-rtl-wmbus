...
var ASM_CONSTS = {
  813112: ($0, $1, $2, $3, $4) => { if (typeof queueBaudrateChange === 'function') return queueBaudrateChange($0, $1, $2, $3, $4); return 0; },  
 813221: ($0) => { return (typeof serialPortReady !== 'undefined' && serialPortReady[$0]) ? 1 : 0; },  
 813305: ($0) => { serialPortReady[$0] = false; },  
 813338: ($0, $1, $2) => { writeSerial($0, $1, $2); },  
 813367: () => { return (typeof pendingDisconnects !== 'undefined') ? pendingDisconnects.length : 0; },  
 813455: ($0) => { return pendingDisconnects[$0]; },  
 813490: ($0) => { pendingDisconnects.splice(0, $0); },  
 813528: () => { return (typeof pendingConnects !== 'undefined') ? pendingConnects.length : 0; },  
 813610: ($0) => { return pendingConnects[$0]; },  
 813642: ($0) => { pendingConnects.splice(0, $0); },  
 813677: ($0) => { try { FS.createDevice('/dev', 'ttyWebUSB' + $0, function() { return null; }, function(c) {} ); } catch(e) { console.warn('[vfs] createDevice /dev/ttyWebUSB' + $0 + ' failed:', e); } },  
 813863: ($0) => { try { FS.unlink('/dev/ttyWebUSB' + $0); } catch(e) {} },  
 813921: ($0) => { if (typeof drainSerialData === 'function') drainSerialData($0); }
};
function __asyncjs__web_download(url_ptr,local_file_ptr,basic_auth_ptr,if_modified_since_ptr) { return Asyncify.handleAsync(async () => { const url = UTF8ToString(url_ptr); const localFile = UTF8ToString(local_file_ptr); const basicAuth = basic_auth_ptr ? UTF8ToString(basic_auth_ptr) : ""; const ifModifiedSince = if_modified_since_ptr ? UTF8ToString(if_modified_since_ptr) : ""; try { const headers = new Headers(); if (basicAuth) { headers.set('Authorization', 'Basic ' + btoa(basicAuth)); } if (ifModifiedSince) { headers.set('If-Modified-Since', ifModifiedSince); } const response = await fetch(url, { method: 'GET', cache: 'no-cache', headers, }); if (response.status === 304) { return 304; } if (!response.ok) { return response.status; } const bytes = new Uint8Array(await response.arrayBuffer()); const normalized = localFile[0] === '/' ? localFile : ('/' + localFile); const lastSlash = normalized.lastIndexOf('/'); const parentDir = lastSlash > 0 ? normalized.slice(0, lastSlash) : '/'; try { FS.unlink(normalized); } catch (e) {} FS.mkdirTree(parentDir); FS.writeFile(normalized, bytes); return response.status || 200; } catch (e) { console.warn(`[download] failed for ${url}:`, e); return -1; } }); }
